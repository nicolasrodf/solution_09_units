...
class _CategoryRouteState extends State<CategoryRoute> {
  ...
  //**** Estos dos reemplazan al initState() ****
  ...
  Future<void> _retrieveLocalCategories() async {
    ...
    data.keys.forEach((key) {
      //Reemplaza el_retrieveUnitList que rellenaba una lista del tipo por ej Area 1, Area 2, etc..
      final List<Unit> units =
          data[key].map<Unit>((dynamic data) => Unit.fromJson(data)).toList();

      var category = Category(
        name: key, //key que reemplaza al "name" que lo recoje del Json. Ya no tenemos el array _categoryNames
        units: units, //unidades reales
        color: _baseColors[categoryIndex],
        iconLocation: Icons.cake,
      );
      //Aca se setean los valores por defecto o actuales que se realizaban en el initState()
      setState(() {
        if (categoryIndex == 0) {
      ...
      categoryIndex += 1; //aumenta el valor para el forEach
    });
  }

  //***De aqui hacia abajo lo mismo de antes**
